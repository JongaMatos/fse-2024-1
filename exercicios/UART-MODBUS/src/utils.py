...
operation_codes = {
    "get_int": [0x23, 0xA1],
    "get_float": [0x23, 0xA2],
    "get_string": [0x23, 0xA3],
    "send_int": [0x16, 0xB1],
    "send_float": [0x16, 0xB2],
    "send_string": [0x16, 0xB3]
}


def cria_buffer(endereço, codigo, matricula, data):

    def argumento_por_codigo():
        if (codigo[1] == operation_codes["send_int"][1]):
            return '<i'
        if (codigo[1] == operation_codes["send_float"][1]):
            return '<f'
        return ''

    tx_buffer = bytearray()
    tx_buffer += bytes([endereço])
    ...
    if (codigo[1] >= operation_codes["send_int"][1]):
        if (codigo == operation_codes["send_string"]):
            ...
            tx_buffer += struct.pack(argumento_por_codigo(), data)
...
def le_float():
    try:
        valor = float(
            input("Digite uma valor numerico decimal: ").strip().replace(',', '.'))
        return valor
    except ValueError:
        print("   Valor invalido")
        return le_float()
...
def le_string():
    try:
        valor = input("Digite uma mensagem de texto: ")
        return valor
    except ValueError:
        print("   Valor invalido")
        return le_string()
